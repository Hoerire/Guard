...
public class MainActivity extends Activity {
    ...
    File cfgFile;
    File logFile;
    static final int LOG_MAX_SIZE=65536; // 64K
    ...
    @Override public void onCreate(Bundle b){
        ...
        logFile=new File(getFilesDir(),"guard.log");
        loadScriptPath();
        buildUi();
        buildHome();
        loadPreviousLogs();
        ...
        new Thread(()->{
            try{
                ...
                for(ApplicationInfo ai:apps){
                    // Guard 自身也加入列表，支持将自身 UI 加入禁用列表
                    // （二进制守护进程独立运行，pm disable 仅禁用 UI，不影响后台服务）
    ...
    // ==================== 日志持久化 ====================
    // 写入日志文件，超过 128K 时自动截断保留后半部分
    void writeLogFile(final String entry){
        synchronized(logLock){
            try{
                FileWriter w=new FileWriter(logFile,true);
                w.append(entry);
                w.close();
                if(logFile.length()>LOG_MAX_SIZE) trimLogFile();
            }catch(Exception ignored){}
        }
    }

    // 截断日志文件：保留后半部分，控制文件大小在 128K 以内
    void trimLogFile(){
        try{
            long len=logFile.length();
            if(len<=LOG_MAX_SIZE) return;
            long start=len/2;
            ...
            String content;
            if(nl>=0&&nl<data.length-1) content=new String(data,nl+1,data.length-nl-1,"UTF-8");
            else content=new String(data,"UTF-8");
